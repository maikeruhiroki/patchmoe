...
論文の4つのデータセット（DRIVE, HV_NIR, Kvasir-SEG, Synapse）に対応
"""
import os
import torch
...
from PatchMoE.real_medical_datasets import build_unified_medical_loader
...
class MedicalDatasetTrainer:
    ...
    def __init__(self, config: dict, device: str = 'cuda:0'):
        ...
        self.output_dir = config.get(
            'output_dir', '/workspace/outputs/patchmoe_medical')
    ...
    def _build_model(self):
        ...
        return PatchMoEModel(
            ...
            num_datasets=self.config.get('num_datasets', 4),
    ...
    def _build_data_loaders(self):
        ...
        # データセット設定
        dataset_configs = [
            {'type': 'drive', 'root_dir': '/workspace/medical_datasets/drive',
                'split': 'train'},
        ...
        val_dataset_configs = [
            {'type': 'drive', 'root_dir': '/workspace/medical_datasets/drive',
                'split': 'val'},
            {'type': 'hv_nir',
                'root_dir': '/workspace/medical_datasets/hv_nir', 'split': 'val'},
            {'type': 'kvasir_seg',
                'root_dir': '/workspace/medical_datasets/kvasir_seg', 'split': 'val'},
            {'type': 'synapse',
                'root_dir': '/workspace/medical_datasets/synapse', 'split': 'val'},
        ]

        # 訓練データローダー
        train_loader = build_unified_medical_loader(
            dataset_configs,
        ...
        # 検証データローダー
        val_loader = build_unified_medical_loader(
            val_dataset_configs,
            batch_size=self.config.get('batch_size', 4),
            num_workers=self.config.get('num_workers', 2),
            shuffle=False,
...
def main():
    parser = argparse.ArgumentParser(
        description='Train PatchMoE on Medical Datasets')
    parser.add_argument('--output_dir', type=str, default='/workspace/outputs/patchmoe_medical',
    ...
    # 訓練実行
    trainer = MedicalDatasetTrainer(config, device=args.device)
    trainer.train()
