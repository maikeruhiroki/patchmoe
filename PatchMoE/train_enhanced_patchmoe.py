#!/usr/bin/env python3
"""
改良されたPatchMoEモデルの訓練スクリプト
論文の設計思想を段階的に実装
"""

from PatchMoE.eval import dice_score, miou
from PatchMoE.enhanced_patchmoe_model import EnhancedPatchMoE, EnhancedLoss
import os
import sys
import torch
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for batch in pbar:
        ...
        try:
            # モデルの出力
            pred_masks, contrastive_loss, load_balancing_loss = model(
                images, dataset_ids, image_ids)

            # 損失計算
            total_loss, loss_dict = criterion(
                pred_masks, masks, contrastive_loss, load_balancing_loss)
...
def main():
    """メイン関数"""
    print("改良されたPatchMoEモデルの訓練を開始します...")
    ...
    # 損失関数とオプティマイザー
    criterion = EnhancedLoss(contrastive_weight=0.1,
    ...
    for epoch in range(num_epochs):
        ...
        if val_dice > best_dice:
            ...
            # モデル保存
            os.makedirs('outputs/enhanced_patchmoe', exist_ok=True)
            torch.save({
                ...
                'best_dice': best_dice,
                'best_miou': best_miou
            }, 'outputs/enhanced_patchmoe/best_model.pt')
    ...
    plot_training_history(
        ...
        'outputs/enhanced_patchmoe/training_history.png'
    )
    ...
    with open('outputs/enhanced_patchmoe/final_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n改良されたPatchMoEモデルの訓練が完了しました！")
