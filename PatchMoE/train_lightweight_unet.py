...
from PatchMoE.lightweight_unet_model import LightweightUNet, LightweightLoss
import os
import sys
import torch
...
class LightweightMedicalDataset(Dataset):
    ...
    def __init__(self, num_samples=100, image_size=256, split='train', num_datasets=4):  # より小さな画像サイズ
    ...
    def _generate_vessel_mask(self):
        ...
        # 複数の血管を生成
        num_vessels = np.random.randint(2, 5)  # より少ない血管数
        for _ in range(num_vessels):
            ...
            # 血管の太さ
            thickness = np.random.randint(1, 4)  # より細い血管

            # 血管を描画
            for t in np.linspace(0, 1, 50):  # より少ない点
    ...
    def _generate_polyp_mask(self):
        ...
        center_y = np.random.randint(
            self.image_size // 4, 3 * self.image_size // 4)
        radius = np.random.randint(10, 30)  # より小さなポリープ
    ...
    def _generate_organ_mask(self):
        ...
        # 複数の臓器を生成
        num_organs = np.random.randint(1, 3)  # より少ない臓器数
        for _ in range(num_organs):
            ...
            center_y = np.random.randint(
                self.image_size // 4, 3 * self.image_size // 4)
            radius = np.random.randint(15, 40)  # より小さな臓器
...
def create_data_loaders():
    """データローダーを作成"""
    train_dataset = LightweightMedicalDataset(
        num_samples=100, split='train', num_datasets=4)
    val_dataset = LightweightMedicalDataset(
        num_samples=25, split='val', num_datasets=4)

    train_loader = DataLoader(
        train_dataset, batch_size=4, shuffle=True, num_workers=0)  # より小さなバッチサイズ
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for batch in pbar:
        ...
        try:
            ...
            # 損失計算
            total_loss, loss_dict = criterion(pred_masks, masks)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_dice > best_dice:
            ...
            # モデル保存
            os.makedirs('outputs/lightweight_unet', exist_ok=True)
            torch.save({
                ...
                'best_dice': best_dice,
                'best_miou': best_miou
            }, 'outputs/lightweight_unet/best_model.pt')
    ...
    plot_training_history(
        ...
        'outputs/lightweight_unet/training_history.png'
    )
    ...
    with open('outputs/lightweight_unet/final_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n軽量U-Netモデルの訓練が完了しました！")
